...
export const AsciiCanvas: React.FC<Props> = ({ params, className = '' }) => {
  ...
  useEffect(() => {
    ...
    const drawPlaceholder = (target: CanvasRenderingContext2D, w: number, h: number) => { const gradient = target.createLinearGradient(0, 0, w, h); gradient.addColorStop(0, '#142b55'); gradient.addColorStop(.45, '#3ca6ff'); gradient.addColorStop(1, '#db6cff'); target.fillStyle = gradient; target.fillRect(0, 0, w, h); target.fillStyle = 'rgba(5,10,20,.45)'; target.beginPath(); target.arc(w * .53, h * .43, Math.min(w, h) * .2, 0, Math.PI * 2); target.fill(); };
